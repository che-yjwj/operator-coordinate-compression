...
TurboQuant는 이 회전 기반 계열과 맞닿아 있으면서도, **온라인 벡터 압축**과 **inner-product distortion 제어**를 전면에 둔다는 점에서 차별적입니다.
랜덤 직교 회전 후 scalar quantization을 수행하고, residual에 대한 1-bit QJL 보정을 결합해
특히 KV cache 같은 long-context serving 병목에 적용됩니다.

---

## 온라인 KV-cache 압축과 inner-product 보정

QJL은 1-bit quantized Johnson-Lindenstrauss transform을 이용해
낮은 비트 예산에서도 inner product estimation을 가능하게 하는 대표적 방법입니다.
TurboQuant는 이 아이디어를 residual 보정에 결합해, 단순 MSE 최소화만으로는 놓치기 쉬운 attention logit bias를 줄이려 합니다.

또한 PolarQuant는 random preconditioning 뒤 polar transformation을 적용해
normalization overhead와 quantization constants 부담을 줄이는 관련 기법군으로 볼 수 있습니다.
이 계열은 모두 "회전 또는 좌표 변환이 압축률과 왜곡 특성을 바꾼다"는 관찰을 공유하지만,
본 연구는 그보다 한 단계 위에서 **어떤 좌표계가 구조적 집중을 유도하는가**를 묻습니다.
...
- TurboQuant류의 강한 온라인 baseline과 manifold-aligned 장기 비전을 분리해 사고하며,
